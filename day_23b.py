    ...
    def run_iter(self):
        while True:
            ...
            if instr[4] == 1 or instr[4] == 2:  # add or multiply
                ...
                if (less_than_flag and first_param < second_param) or (
                        not less_than_flag and first_param == second_param):
...
def run_network(memory_list):
    ...
    cur_packet_nat = (None, None)
    last_packet_nat = (None, None)
    nat_address = 255

    how_many_iter = 4
    was_idle_last = [False] * how_many_iter
    ...
    while True:
        ...
        if is_idle and was_idle_last.count(True) == how_many_iter:
            if cur_packet_nat[1] == last_packet_nat[1]:
                print("The first repeated Y value is", cur_packet_nat[1])
                break
            network[0].input_queue.append(cur_packet_nat[0])
            network[0].input_queue.append(cur_packet_nat[1])
            last_packet_nat = cur_packet_nat
            was_idle_last = [False] * how_many_iter

        was_idle_last = was_idle_last[1:] + [is_idle]
